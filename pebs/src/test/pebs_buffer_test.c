/*
 * @Date: 2023-05-19 17:08:49
 * @LastEditors: liuchang chang.liu@zhejianglab.com
 * @LastEditTime: 2023-07-12 17:44:16
 * @FilePath: /pebs/src/test/pebs_buffer_test.c
 */
...
MODULE_DESCRIPTION("pebs buffer test");
MODULE_VERSION("1.0");
...
int __init pebs_buffer_init(void) {
    alloc_ring_buffer();

    uint64_t start_addr = 0x234f403;
    uint64_t end_addr = 0x234f443;

    write_ring_buffer(0,start_addr,end_addr);
    write_ring_buffer(1,start_addr,end_addr);
    write_ring_buffer(2,start_addr,end_addr);
    write_ring_buffer(3,start_addr,end_addr);
    write_ring_buffer(4,start_addr,end_addr);
    write_ring_buffer(5,start_addr,end_addr);
    write_ring_buffer(6,start_addr,end_addr);
    //write_ring_buffer(7);
    //write_ring_buffer(8);
    ...
    for(; index < num; index ++){
        uint32_t pid;
        uint64_t start_addr_0=0;
        uint64_t end_addr_0=0;
        while(read_ring_buffer(index, &pid, &start_addr_0, &end_addr_0) == 0){
            
            printk(KERN_INFO "buffer_id_%d:read from buffer, pid=%d, start_addr=%llx, end_addr=%llx.\n", index, pid, start_addr_0, end_addr_0);
            //kill_pid(find_vpid((int32_t)pid),SIGKILL,1);
        }
    }
...
void __exit pebs_buffer_exit(void) 
{   
    free_ring_buffer();
}

module_init(pebs_buffer_init); 
module_exit(pebs_buffer_exit); 
MODULE_LICENSE("GPL");